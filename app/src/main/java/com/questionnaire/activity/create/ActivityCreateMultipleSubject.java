...
public class ActivityCreateMultipleSubject extends Activity
	...
	final int[] items = {
			...
			R.id.subject_value_5,
			R.id.subject_value_6,
			R.id.subject_value_7};
	...
	private void initView(){
		...
		setContentView(R.layout.activity_create_multiple_subject);
	...
	private void initData(){
		...
		if(i.hasExtra(Conf.INTENT_TYPE))
			type = i.getIntExtra(Conf.INTENT_TYPE, -1);
		if(i.hasExtra(Conf.INTENT_SUBJECT)) 
			mSubject = (Subject) i.getSerializableExtra(Conf.INTENT_SUBJECT);
		if(mPaperId < 0 || mPaperNumber < 0 || type < 0){
